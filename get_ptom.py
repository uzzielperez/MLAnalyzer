...
def main():
    ...
    h_EB_ptom = ROOT.TH1F('h_EB_ptom', 'h_EB_ptom;energy/m_{#gamma#gamma};N_{hits}', 50, 0., 5.)
    ...
    #hFile = ROOT.TFile("h_EB_ptom_GluGluH2GG.root","RECREATE")
    hFile = ROOT.TFile("h_EB_ptom_%s.root"%(ggInStr.split('/')[-1].split('_')[0]), "RECREATE")
    h_EB_ptom.Write()
    hFile.Close()
