...
for j,decay in enumerate(decays):
    ...
    nbins = 20
    if j == 2:
      h, xs = np.histogram(y_mass0.compute(), bins=nbins, range=[0.,1.6]) 
      print(h)
      h = 1.*h/h.sum()
      print(h)
      lhood = 1./h
      ...
      wgt = da.from_array(np.ones_like(y_mass0), chunks=(chunk_size,))
    ...
    file_out_str = "%s/%s_IMGcropV3_RH%d_n%dkx2_wgt.hdf5"%(eosDir,decay,int(scale),neff//1000.)
