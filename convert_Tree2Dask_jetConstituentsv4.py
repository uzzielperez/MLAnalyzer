...
for d,decay in enumerate(decays):
  ...
  tfile_idxs = glob.glob('%s/%s*_AODSIM/180919_131137/*/output_*.root'%(eosDir,decay))
  ...
  for n in tfile_idxs:
    ...
    tfile_str = glob.glob('%s/%s*_AODSIM/180919_131137/*/output_%s.root'%(eosDir,decay,n))[0]
    ...
    for ijet in [0]: # only keep leading jet
      ...
      f = h5py.File('%s/%s_FC/%s_label%d_ijet%d_%s.hdf5'%(eosDir, decay, decay, d, ijet, n))
