...
for j,decay in enumerate(decays):
  tfile_idxs = glob.glob('%s/%s*_AODSIM/*/*/output_*.root'%(eosDir,decay))
  ...
  for n in tfile_idxs:
    ...
    #for ijet in [1]:
      ...
      tfile_str = glob.glob('%s/%s*_AODSIM/*/*/output_%s.root'%(eosDir,decay,n))[0]
      tfile = ROOT.TFile(tfile_str)
      tree = tfile.Get('fevt/RHTree')
      nevts = tree.GetEntries()
      print " >> Input file:", tfile_str

      #tfiles = glob.glob('%s/%s*_AODSIM/*/*/output_%s.root'%(eosDir,decay,n))
      ...
      n_jets = (len(jetMask[jetMask==True])//chunk_size_)*chunk_size_
      ...
      file_out_str = "%s/%s_IMGjet_RH%d_n%d_label%d_jet%d_list%s_%s.hdf5"%(eosDir,decay,int(scale[0]),n_jets,label,ijet,list_idx,n)
