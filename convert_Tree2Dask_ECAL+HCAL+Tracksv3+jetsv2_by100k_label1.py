...
#decays = ['dummy']
#decays = ['QCD_Pt_80_170', 'QCD_Pt_80_170']
list_idx = '01000'
...
for j,decay in enumerate(decays):
  ...
  procs = range(0, nevts, int(1e5))
  for it, start in enumerate(procs):
    ...
    if it == len(procs)-1:
      ...
      neff = int(1e5)
    #neff = 5000
    print " >> Doing: [%d, %d):"%(start, start+neff)
    ...
    start_neff = start+neff
    proc_range = range(start, start_neff, chunk_size)
    #print " >> proc range:", proc_range
    ...
    eventId = da.concatenate([\
                da.from_delayed(\
                    load_single(tree,i,i+get_chunk_size(i, start_neff, chunk_size), branches),\
                    shape=(get_chunk_size(i, start_neff, chunk_size),),\
                    dtype=np.int32)\
                for i in proc_range])
    ...
    X_ECAL = da.concatenate([\
                da.from_delayed(\
                    load_X(tree,i,i+get_chunk_size(i, start_neff, chunk_size), branches, readouts, scale[0]),\
                    shape=(get_chunk_size(i, start_neff, chunk_size), readouts[0], readouts[1], len(branches)),\
                    dtype=np.float32)\
                for i in proc_range])
    ...
    X_HBHE_up = da.concatenate([\
                da.from_delayed(\
                    load_X_upsampled(tree,i,i+get_chunk_size(i, start_neff, chunk_size), branches, readouts, scale[1], upscale),\
                    shape=(get_chunk_size(i, start_neff, chunk_size), readouts[0]*upscale, readouts[1]*upscale, len(branches)),\
                    dtype=np.float32)\
                for i in proc_range])
    ...
    #for ijet in [1]:
      ...
      file_out_str = "%s/%s_IMGjet_RH%d_n%dk_label%d_jet%d_it%d.hdf5"%(eosDir,decay,int(scale[0]),neff//1000,label,ijet,it)
