...
def zero_out(img, iphi, ieta):

    """
    Zero out the image outside the two windows of the two jets
    windows size defined by jet_shape (global var.)
    expect iphi=[jetSeed_iphi0,jetSeed_iphi1], and
    ieta=[jetSeed_ieta0,jetSeed_ieta1]
    """
    # zeroImage = np.zeros(img.shape)

    zero_mask = np.zeros((280,360), dtype=bool)
    iphi = [int(phi * 5 + 2) for phi in iphi]
    ieta = [int(eta * 5 + 2) for eta in ieta]
    off = jet_shape//2
    for ijet, phi in enumerate(iphi):
        eta = ieta[ijet]

        if phi < off:
            diff = off-phi
            aSlice = np.s_[eta-off:eta+off+1,-diff:]
            anotherSlice = np.s_[eta-off:eta+off+1,:phi+off+1]

        elif 360 - phi < off:
            diff = off - (360-phi)
            aSlice = np.s_[eta-off:eta+off+1,phi-off:]
            anotherSlice = np.s_[eta-off:eta+off+1,:diff+1]

        else:
            aSlice = np.s_[eta-off:eta+off+1,phi-off:phi]
            anotherSlice = np.s_[eta-off:eta+off+1,phi:phi+off+1]

        zero_mask[aSlice] = True
        zero_mask[anotherSlice] = True
    img[~zero_mask] = 0.
    #del zero_mask
    return img

@delayed
def zero_out_block(Xs, iphi0s, iphi1s, ieta0s, ieta1s):
    return np.array([zero_out(x, [iphi0, iphi1], [ieta0, ieta1]) for x, iphi0, iphi1, ieta0, ieta1 in zip (Xs, iphi0s, iphi1s, ieta0s, ieta1s)])
...
for d,decay in enumerate(decays):
    ...
    #tfile_idxs = glob.glob('%s/%s*_AODSIM/%s/*/output_6.root'%(eosDir,decay,date_str))
    ...
    for n in tfile_idxs:
        ...
        ijet = 1 # only keep leading jet

        print ' >> ijet:',ijet

        # jet seed iphi
        branches = ["jetSeed_iphi%d"%ijet]
        jetSeed_iphi1_ = da.concatenate([\
                    da.from_delayed(\
                        load_single(tree,i,i+get_chunk_size(i, neff, chunk_size), branches),\
                        shape=(get_chunk_size(i, neff, chunk_size),),\
                        dtype=np.float32)\
        ...
        # Zero out
        X_ECAL_stacked_zeroed_ = da.concatenate([\
                  da.from_delayed(\
                      zero_out_block(X_ECAL_stacked_[i:i+get_chunk_size(i, neff, chunk_size)],\
                      jetSeed_iphi0_[i:i+get_chunk_size(i, neff, chunk_size)], jetSeed_iphi1_[i:i+get_chunk_size(i, neff, chunk_size)],\
                      jetSeed_ieta0_[i:i+get_chunk_size(i, neff, chunk_size)], jetSeed_ieta1_[i:i+get_chunk_size(i, neff, chunk_size)]),\
                      shape=(get_chunk_size(i, neff, chunk_size), X_ECAL_stacked_.shape[1], X_ECAL_stacked_.shape[2], 3),\
                      dtype=np.float32)\
                  for i in proc_range])
        ...
        jetSeed_iphi0 = jetSeed_iphi0_[jetMask]
        print "  >> %s: %s"%('jetSeed_iphi', jetSeed_iphi0.shape)
        jetSeed_ieta0 = jetSeed_ieta0_[jetMask]
        print "  >> %s: %s"%('jetSeed_ieta', jetSeed_ieta0.shape)

        jetSeed_iphi1 = jetSeed_iphi1_[jetMask]
        print "  >> %s: %s"%('jetSeed_iphi', jetSeed_iphi1.shape)
        jetSeed_ieta1 = jetSeed_ieta1_[jetMask]
        print "  >> %s: %s"%('jetSeed_ieta', jetSeed_ieta1.shape)
        X_ECAL_stacked = X_ECAL_stacked_[jetMask]
        print "  >> %s: %s"%('X_ECAL_stacked', X_ECAL_stacked.shape)
        X_ECAL_stacked_zeroed = X_ECAL_stacked_zeroed_[jetMask]
        print "  >> %s: %s"%('X_ECAL_stacked_zeroed', X_ECAL_stacked_zeroed.shape)
        ...
        da.to_hdf5(file_out_str, {
                                  ...
                                  'X_ECAL_stacked_zeroed': X_ECAL_stacked_zeroed,
                                  #'y': y,
                                  'jetRunId': jetRunId,
