...
list_idx = '00000'
#list_idx = '00001'
date_str = '181203_095043' #'181101_152041' #'181029_120833' #00000
#date_str = '181203_094914' #'181101_152128' #'181025_233834' #00001
...
for d,decay in enumerate(decays):
    ...
    #tfile_idxs = glob.glob('%s/%s*_AODSIM/%s/*/output_64.root'%(eosDir,decay,date_str))
    tfile_idxs = [s.replace('.root','').split('_')[-1] for s in tfile_idxs]
    tfile_idxs = [int(i) for i in tfile_idxs]
    tfile_idxs.sort()
    #tfile_idxs = np.arange(2)
    print tfile_idxs
    ...
    for n in tfile_idxs:
        ...
        # FC inputs 
        branches = ["Dijet_inputs"]
        X_FC_ = da.concatenate([\
                    da.from_delayed(\
                        load_single(tree,i,i+get_chunk_size(i, neff, chunk_size), branches),\
                        shape=(get_chunk_size(i, neff, chunk_size),5),\
                        dtype=np.float32)\
                    for i in range(0,neff,chunk_size)])
        print " >> Expected shape:", X_FC_.shape
        ...
        #branches = ["ECAL_EndtracksPt", "ECAL_tracksPt"]
        branches = ["ECAL_genHadPartPt", "ECAL_genEMPartPt", "ECAL_tracksPt"]
        ...
        print " >> %s: %s"%('X_ECAL_stacked_', X_ECAL_stacked_.shape)
        ...
        for ijet in [0, 1]:
            ...
            X_jets_ = da.concatenate([\
                        da.from_delayed(\
                            crop_jet_block(X_ECAL_stacked_[i:i+get_chunk_size(i, neff, chunk_size)],\
                            ...
                            shape=(get_chunk_size(i, neff, chunk_size), jet_shape, jet_shape, X_ECAL_stacked_.shape[-1]),\
                            dtype=np.float32)\
                        for i in proc_range])
            ...
            outPath = '%s/%s_IMGjetv2'%(outDir, decay)
            if not os.path.isdir(outPath):
                os.makedirs(outPath)
            file_out_str = "%s/%s_IMGjet_RH%d_n%d_label%d_jet%d_list%s_%d.hdf5"%(outPath,decay,int(scale[0]),n_jets,label,ijet,list_idx,n)
            #file_out_str = "%s_IMGjet_RH%d_n%d_label%d_jet%d_list%s_%d.hdf5"%(decay,int(scale[0]),n_jets,label,ijet,list_idx,n)
