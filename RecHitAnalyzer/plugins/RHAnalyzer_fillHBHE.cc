...
void RecHitAnalyzer::fillHBHE ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  vHBHE_energy_EB_.assign( 2*hcaldqm::constants::IPHI_NUM*HBHE_IETA_MAX_EB, 0. );
  vHBHE_energy_.assign( 2*hcaldqm::constants::IPHI_NUM*(hcaldqm::constants::IETA_MAX_HE-1), 0. );
  ...
  */
  ...
  for ( HBHERecHitCollection::const_iterator iRHit = HBHERecHitsH_->begin();
    ...
    idx_ = ( ieta_+HBHE_IETA_MAX_EB )*hcaldqm::constants::IPHI_NUM + iphi_;
  ...
  for (int ieta = 1; ieta < hEvt_HBHE_energy->GetNbinsY()+1; ieta++) {
    for (int iphi = 1; iphi < hEvt_HBHE_energy->GetNbinsX()+1; iphi++) {
      ...
      if ( energy_ == 0. ) continue;
      idx_ = (ieta-1)*hcaldqm::constants::IPHI_NUM + (iphi-1);
      // Fill vector for image
