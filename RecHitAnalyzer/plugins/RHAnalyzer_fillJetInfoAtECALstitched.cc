...
void fillTracksAtECAL_with_EEproj ( TH2F *hEvt_EE_tracksd0_, /*, TH2F *hEvt_EE_tracksQPt_,*/ int ieta_global_offset, int ieta_signed_offset ) {
...
//  //float trackQPt_;
//
//  for (int ieta = 1; ieta < hEvt_EE_tracksd0_->GetNbinsY()+1; ieta++) {
//    ieta_ = ieta - 1;
//    ieta_global_ = ieta_ + ieta_global_offset;
//    ieta_signed_ = ieta_ + ieta_signed_offset;
//    for (int iphi = 1; iphi < hEvt_EE_tracksd0_->GetNbinsX()+1; iphi++) {
//
...
void RecHitAnalyzer::fillJetInfoAtECALstitched ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  for ( int iz(0); iz < nEE; ++iz ) hEvt_EE_tracksd0[iz]->Reset();
